...
public abstract class GameObj {
    ...
    public boolean intersectsSnakeCoords(CoordinatePair p, Square snake) {
        return (this.px + this.width >= p.x()
            && this.py + this.height >= p.y()
            && p.x() + snake.getWidth() >= this.px 
            && p.y() + snake.getHeight() >= this.py);
    }
    
    public boolean intersectsSnakeBody(Square snake) {
        LinkedList<CoordinatePair> bodyPositions = snake.getBodyPositions();
        bodyPositions.removeFirst();
        //Direction dir = this.getDirection();
        for (CoordinatePair p : bodyPositions) {
            if (intersectsSnakeCoords(p, snake)) {
                return true;
            }
        }
        return false;
